...
import { InviteToRoomModal } from "@/components/chat/invite-to-room-modal"
import { JoinPublicRoomsModal } from "@/components/chat/join-public-rooms-modal"
...
export default function ChatInterface({ currentUser, initialRooms }: ChatInterfaceProps) {
  ...
  const [isJoinPublicRoomsOpen, setIsJoinPublicRoomsOpen] = useState(false)
  const [isInviteToRoomOpen, setIsInviteToRoomOpen] = useState(false)
  const [roomToInviteTo, setRoomToInviteTo] = useState<Room | null>(null)
  ...
  // Handle invitation to a room
  const handleInviteToRoom = (room: Room) => {
    setRoomToInviteTo(room)
    setIsInviteToRoomOpen(true)
  }
  ...
  const pendingRequests = filteredRooms.filter(
    (room) => isDoctor && room.status === "pending" && room.room_type === "1:1" && room.doctor_id === currentUser.id,
  )
  ...
  // Add this function after the `getOtherParticipant` function
  const isPublicRoom = (room: Room) => {
  ...
  const getAvatarStackData = () => {
    return roomParticipants
      ...
      .map((profile) => ({
        name: profile.first_name ? `${profile.first_name} ${profile.last_name || ""}` : profile.username || "Unknown",
    ...
    <div className="flex flex-col h-full w-full bg-background">
          ...
          <div className="container mx-auto flex items-center justify-between">
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium text-amber-800 dark:text-amber-300">
                You have {pendingRequests.length} pending message{" "}
      ...
      <div className="flex items-center justify-between p-3 md:p-4 border-b border-border sticky top-0 bg-background/95 backdrop-blur-sm z-10 shadow-sm">
        {selectedRoom ? (
          <div className="flex items-center flex-1 overflow-hidden gap-2 md:gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {selectedRoom && isPublicRoom(selectedRoom) && (
                <Button
                  variant="outline"
                  size="sm"
                  className="ml-auto text-xs"
                  onClick={() => handleInviteToRoom(selectedRoom)}
                >
        ...
        <div className="flex items-center gap-1 md:gap-2">
          ...
          {isDoctor && (
            <Button size="sm" variant="outline" onClick={() => setIsCreateRoomOpen(true)} className="hidden md:flex">
            ...
            <PopoverTrigger asChild>
              ...
              >
                <Avatar className="h-7 w-7">
                  ...
                  <AvatarFallback>{currentUser.first_name?.[0] || currentUser.username?.[0] || "U"}</AvatarFallback>
                </Avatar>
              </Button>
            </PopoverTrigger>
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="p-4 border-b border-border space-y-2">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {selectedRoom.is_private && <Badge variant="outline">Private</Badge>}
                    </div>
                    {selectedRoom && isPublicRoom(selectedRoom) && (
                      <Button
                        variant="outline"
          ...
          <div className="flex-1 overflow-y-auto p-2">
            ...
            {isDoctor && pendingRequests.length > 0 && (
              <div
                ...
                className="mb-4 pt-4 border-t-2 border-amber-200 dark:border-amber-800 bg-amber-50/30 dark:bg-amber-950/10 rounded-md"
              >
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex gap-1">
                      ...
                      {room.is_private && (
                        <div className="bg-primary/10 text-primary text-xs px-2 py-0.5 rounded-full">Private</div>
                      )}
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  {isDoctor ? (
                    ...
                    <>
                      ...
                      <Button onClick={() => setIsJoinPublicRoomsOpen(true)} variant="outline">
                        <Users className="h-4 w-4 mr-1" />
                        Join Group Therapy
                      </Button>
                    </>
      ...
      {/* Join public rooms modal */}
      <JoinPublicRoomsModal
        isOpen={isJoinPublicRoomsOpen}
        onClose={() => setIsJoinPublicRoomsOpen(false)}
        currentUserId={currentUser.id}
        userRoomIds={userRoomIds}
        onRoomJoined={addNewRoom}
      />
      ...
      {roomToInviteTo && (
        <InviteToRoomModal
          isOpen={isInviteToRoomOpen}
          onClose={() => {
            setIsInviteToRoomOpen(false)
            setRoomToInviteTo(null)
          }}
          room={roomToInviteTo}
          currentUserId={currentUser.id}
        />
